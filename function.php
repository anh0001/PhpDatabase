...
function simpan_data_pegawai() {
	...
	if( !count( $salah ) ) {
		...
		if( itung_jumlahnya( mysql_query( "SELECT * FROM pegawai WHERE kode='$kode'" ) ) == 0 ) {
			mysql_query( "INSERT INTO pegawai VALUES( '', '$kode', '$nama', '$alamat', '$gaji', '$sex', '$status', '$jabatan', '$new_image_name', '".time()."' )" );
...
function update_data_pegawai() {
	...
	# jika tidak ada kesalahan
	if( !count( $salah ) ) {		
		$sql = uraikan(mysql_query("SELECT * FROM pegawai WHERE kode='{$kode}'"));
		$sex = ($sex != '') ? $sex : $sql['sex'];
		$jabatan = ($jabatan != '') ? $jabatan : $sql['jabatan'];
		$status = ($status != '') ? $status : $sql['status'];
		$new_image_name = ($filename != '') ? $new_image_name : $sql['photo'];
		mysql_query( "UPDATE pegawai SET kode='$kode', nama='$nama', alamat='$alamat', gaji='$gaji', sex='$sex', status='$status', jabatan='$jabatan', photo='$new_image_name' WHERE kode='$kode'" );
		move_uploaded_file($_FILES['photo']['tmp_name'], $dir . "/" . $new_image_name);
	}
	# jika ada kesalahan simpan aja di dalam session
	if( count( $salah ) ) {
...
function hapus_data_pegawai() {
	...
	mysql_query( "DELETE FROM pegawai WHERE kode='$kode'" );
	header( "Location: ".URL."/?option=data-pegawai" );
	exit;
}
...
function cari_terbesar( $kode ) {
	$query = uraikan( mysql_query( "SELECT MAX(kode) AS terbesar FROM pegawai" ) );
	$kode = $query['terbesar'];
	if( $kode ) {
...
function tampilkan_pegawai() {
	...
	$sql = mysql_query( "SELECT * FROM pegawai ORDER BY kode ASC LIMIT $offset,$jumlah_data_per_halaman" );
	$jumlah_data_pegawai = itung_jumlahnya(mysql_query("SELECT * FROM pegawai"));	
	echo "<div class=\"box\">\n";
	...
	echo "</table>";	
	$jumlah_data = uraikan( mysql_query("SELECT COUNT(*) AS jumlah FROM pegawai") );
